...
void * thread_c(void * arg_in)
{
    ...
    // TODO
    //  send max value
    check_pthread_return(pthread_mutex_lock(&arg->mutex), "pthread_mutex_lock");
    arg->max = gmn_get_max();
    arg->status = S_MAX;
    check_pthread_return(pthread_cond_signal(&arg->cond_result), "pthread_cond_signal");
    check_pthread_return(pthread_mutex_unlock(&arg->mutex), "pthread_mutex_unlock");
    ...
    for (;;) {
        ...
        check_pthread_return(pthread_cond_wait(&arg->cond_guess, &arg->mutex), "pthread_cond_wait");
        }
        guess = arg->guess;
        check_pthread_return(pthread_mutex_unlock(&arg->mutex), "pthread_mutex_unlock");
    
    //      call gmn_check()
        r = gmn_check(&gmn, guess);

    //      send the result and, if the guess is correct, the final message to thread_p
        check_pthread_return(pthread_mutex_lock(&arg->mutex), "pthread_mutex_lock");
        arg->result = r;
        if (r == 0) {
            snprintf(arg->message, MSG_BUF_SIZE, "%s", gmn_get_message(&gmn));
        }
        arg->status = S_RESULT;
        check_pthread_return(pthread_cond_signal(&arg->cond_result), "pthread_cond_signal");
        check_pthread_return(pthread_mutex_unlock(&arg->mutex), "pthread_mutex_unlock");
...
void * thread_p(void *arg_in)
{
    ...
    while (arg->status != S_MAX) {
        check_pthread_return(pthread_cond_wait(&arg->cond_result, &arg->mutex), "pthread_cond_wait");
    }
    max = arg->max;
    check_pthread_return(pthread_mutex_unlock(&arg->mutex), "pthread_mutex_unlock");
    ...
    do { 
        ...
        arg->guess = guess;
        arg->status = S_GUESS;
        check_pthread_return(pthread_cond_signal(&arg->cond_guess), "pthread_cond_signal");
...
int main(int argc, char *argv[])
{
    ...
    check_pthread_return(pthread_mutex_init(&arg.mutex, NULL), "pthread_mutex_init");
    check_pthread_return(pthread_cond_init(&arg.cond_guess, NULL), "pthread_cond_init");
    check_pthread_return(pthread_cond_init(&arg.cond_result, NULL), "pthread_cond_init");

    pthread_t tc, tp;
    check_pthread_return(pthread_create(&tc, NULL, thread_c, &arg), "pthread_create C");
    check_pthread_return(pthread_create(&tp, NULL, thread_p, &arg), "pthread_create P");

    check_pthread_return(pthread_join(tp, NULL), "pthread_join P");
    check_pthread_return(pthread_join(tc, NULL), "pthread_join C");

    check_pthread_return(pthread_cond_destroy(&arg.cond_result), "pthread_cond_destroy");
    check_pthread_return(pthread_cond_destroy(&arg.cond_guess), "pthread_cond_destroy");
    check_pthread_return(pthread_mutex_destroy(&arg.mutex), "pthread_mutex_destroy");
