...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
        ...
        int old = p[i].loc;
        int newloc = (old + steps) % n;

        if (old + steps >= n) {        // passed or landed on 0
            p[i].balance += n;
        }

        p[i].loc = newloc;

        int owner = prop[newloc].owner_id;
        if (owner == -1) {
            prop[newloc].owner_id = p[i].id;          // claim unowned
        } else if (owner != p[i].id) {
            int rent = prop[newloc].rent;
            if (!transaction(&p[i], &p[owner], rent)) // couldn’t pay
                return 0;                              // game ends
        }
    }
	return 1;
